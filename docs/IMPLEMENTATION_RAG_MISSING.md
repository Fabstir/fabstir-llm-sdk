# Implementation Plan: UI Features for RAG System

## Implementation Status

**STATUS**: ⏳ **NOT STARTED** (0% complete)
**Start Date**: TBD
**Target Completion**: TBD (Estimated: 22-37 weeks)

**Scope**: This plan covers all missing UI organizational features required by `docs/ui4-reference/UI_MOCKUPS.md`:
- Session Groups (Claude Projects-style organization)
- Vector Database Management UI
- RAG Sources Transparency
- Sharing & Collaboration
- Dashboard & Navigation
- Settings & Advanced Features

**Note**: Backend RAG functionality (document upload, vector search, embeddings) is **100% complete** and tracked in `IMPLEMENTATION_RAG.md`.

## Overview

Extend the fabstir-llm-sdk with a comprehensive UI layer that transforms the current single-session chat interface into a fully-featured knowledge management system. This implementation will provide users with tools to organize their AI interactions into projects (Session Groups), manage document libraries, collaborate with others, and gain transparency into RAG decision-making.

## Goals

Deliver a production-ready UI that:
1. **Organizes conversations** into Session Groups (like Claude Projects)
2. **Manages vector databases** with folder hierarchies and file browsing
3. **Provides RAG transparency** showing which documents influenced responses
4. **Enables collaboration** with granular sharing permissions
5. **Simplifies navigation** with dashboards and quick actions
6. **Supports mobile users** with responsive layouts
7. **Maintains data sovereignty** through S5 storage and encryption
...
│                     UI Layer (New)                                  │
│  ┌──────────────────────────────────────────────────────────────┐  │
│  │  Session Groups UI        Vector DB Management UI            │  │
│  │  • Group list view        • Database list with stats         │  │
│  │  • Group detail (3-col)   • Folder tree navigation           │  │
│  │  • Create/edit groups     • File browser                     │  │
│  │  • Link databases         • Search within databases          │  │
│  │                                                               │  │
│  │  RAG Sources UI           Dashboard & Navigation             │  │
│  │  • Sources panel          • Home with quick stats            │  │
│  │  • Similarity scores      • Recent databases                 │  │
│  │  • View excerpts          • Quick actions FAB                │  │
│  │  • Document paths         • Settings page                    │  │
│  │                                                               │  │
│  │  Collaboration UI         Mobile Responsive                  │  │
│  │  • Share modal            • Collapsible sidebars             │  │
│  │  • Permissions            • Touch-optimized                  │  │
│  │  • Invitations            • Mobile navigation                │  │
│  └──────────────────────────────────────────────────────────────┘  │
└───────────────────────────┬─────────────────────────────────────────┘
                            │
┌───────────────────────────▼─────────────────────────────────────────┐
...
│  • VectorRAGManager (vector CRUD)                                   │
│  • DocumentManager (chunking, embedding)                            │
...
└───────────────────────────┬─────────────────────────────────────────┘
                            │
┌───────────────────────────▼─────────────────────────────────────────┐
│                   New SDK Managers (Phase 1)                        │
│  • SessionGroupManager - Manage groups of related sessions          │
│  • PermissionManager - Sharing and access control                   │
└─────────────────────────────────────────────────────────────────────┘
```
...
├── managers/
│   ├── SessionGroupManager.ts           (Phase 1.1) - NEW
│   ├── PermissionManager.ts             (Phase 5.1) - NEW
│   ├── SessionManager.ts                (Phase 1.3) - MODIFIED
│   └── VectorRAGManager.ts              (Phase 1.4) - MODIFIED
│
├── interfaces/
│   ├── ISessionGroupManager.ts          (Phase 1.1) - NEW
│   └── IPermissionManager.ts            (Phase 5.1) - NEW
│
├── types/
│   ├── session-groups.types.ts          (Phase 1.1) - NEW
│   └── permissions.types.ts             (Phase 5.1) - NEW
│
└── storage/
    └── SessionGroupStorage.ts           (Phase 1.2) - NEW (S5 persistence)

apps/harness/
├── pages/
│   ├── dashboard.tsx                    (Phase 6.1) - NEW
│   ├── vector-databases.tsx             (Phase 2.1) - NEW
│   ├── session-groups.tsx               (Phase 4.1) - NEW
│   ├── session-group-detail.tsx         (Phase 4.3) - NEW
│   └── settings.tsx                     (Phase 7.1) - NEW
│
└── components/
    ├── session-groups/
    │   ├── SessionGroupList.tsx         (Phase 4.1) - NEW
    │   ├── SessionGroupCard.tsx         (Phase 4.1) - NEW
    │   ├── CreateSessionGroupModal.tsx  (Phase 4.2) - NEW
    │   └── SessionGroupDashboard.tsx    (Phase 4.3) - NEW
    │
    ├── vector-databases/
    │   ├── DatabaseList.tsx             (Phase 2.1) - NEW
    │   ├── DatabaseCard.tsx             (Phase 2.1) - NEW
    │   ├── FolderTree.tsx               (Phase 2.2) - NEW
    │   └── FileBrowser.tsx              (Phase 2.3) - NEW
    │
    ├── rag-sources/
    │   ├── SourcesPanel.tsx             (Phase 3.1) - NEW
    │   ├── ViewSourcesModal.tsx         (Phase 3.2) - NEW
    │   └── DocumentPathBreadcrumb.tsx   (Phase 3.3) - NEW
    │
    ├── sharing/
    │   ├── ShareModal.tsx               (Phase 5.2) - NEW
    │   ├── CollaboratorList.tsx         (Phase 5.3) - NEW
    │   └── InvitationManager.tsx        (Phase 5.4) - NEW
    │
    ├── dashboard/
    │   ├── QuickStats.tsx               (Phase 6.1) - NEW
    │   ├── RecentDatabasesGrid.tsx      (Phase 6.2) - NEW
    │   ├── ActiveSessionGroupsList.tsx  (Phase 6.3) - NEW
    │   └── QuickActionsPanel.tsx        (Phase 6.4) - NEW
    │
    └── common/
        ├── QuickActionsFAB.tsx          (Phase 6.4) - NEW
        └── NotificationCenter.tsx       (Phase 7.3) - NEW
```
